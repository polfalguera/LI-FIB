...
eachCourseExactlyOneProf:- 
    courseProfessors(C,LP), 
    findall( cp(C,P), member(P,LP), Lits ), 
    exactly(1,Lits), 
    fail.
eachCourseExactlyOneProf.
...
eachCourseExactlyOneRoom:- 
    courseRooms(C,LR), 
    findall( cr(C,R), member(R,LR), Lits ), 
    exactly(1,Lits), 
    fail.
eachCourseExactlyOneRoom.
...
noOverlapCoursesSameYear:- 
    course(C1), course(C2),
    courseYear(C1,Y), courseYear(C2,Y),
    C1 \= C2,
    day(D), hour(H),
...
noOverlapProfs:- 
    courseProfessors(C1,LP1), courseProfessors(C2,LP2),
    C1 \= C2,
    member(P,LP1), member(P,LP2),
    day(D), hour(H),    
...
noOverlapRooms:-
    courseRooms(C1,LR1), courseRooms(C2,LR2),
    C1 \= C2,
    member(R,LR1), member(R,LR2),
    day(D), hour(H),    
...
relateVarsCPwithUsedProf:- 
    professor(P),
    findall(cp(C,P), (courseProfessors(C,LP),member(P,LP)), Lits),
    expressOr(usedProf(P), Lits),
    fail.
relateVarsCPwithUsedProf.
