...
satVariable( match(S,T,R) ):- round(R), difTeams(S,T).
% match-S-T-R    meaning  "on round R there is a match S-T at home of S"
satVariable( home(S,R) ):- team(S), round(R).
% home-S-R       meaning  "team S plays at home on round R"
satVariable( double(S,R) ):- team(S), round(R).
...
writeClauses(infinite):- !, writeClauses(20),!.
writeClauses(MaxCost):-
    eachTeamEachRoundExactlyOneMatch,
    oneTeamAgainstAnotherExactlyOneRound,
    defineHome,
    defineDouble,
    noTriples,
    eachRoundAtLeastOneTvMatch,
    notAtHome,
    noDoublesAtCertainRounds,
    maxCost(MaxCost),
    true,!.
writeClauses(_):- told, nl, write('writeClauses failed!'), nl,nl, halt.

eachTeamEachRoundExactlyOneMatch:- team(T), round(R), 
                                    findall(match(S,T,R), difTeams(S,T), Lits1), 
                                    findall(match(T,S,R), difTeams(S,T), Lits2), 
                                    append(Lits1,Lits2,Lits),
                                    exactly(1,Lits), fail.
eachTeamEachRoundExactlyOneMatch.

oneTeamAgainstAnotherExactlyOneRound:- difTeams(S,T), 
                                        findall(match(S,T,R), round(R), Lits1), 
                                        findall(match(T,S,R), round(R), Lits2), 
                                        append(Lits1,Lits2,Lits),
                                        exactly(1,Lits), fail.
oneTeamAgainstAnotherExactlyOneRound.
...
defineDouble:- team(T), round(R), round(R2), R2 is R+1, 
                writeClause([-home(T,R), -home(T,R2), double(T,R2)]),
                writeClause([home(T,R), home(T,R2), double(T,R2)]),
                writeClause([-home(T,R), home(T,R2), -double(T,R2)]),
                writeClause([home(T,R), -home(T,R2), -double(T,R2)]), fail.
defineDouble.  

noTriples:- team(T), round(R), round(R2), R2 is R+1, writeClause([-double(T,R), -double(T,R2)]), fail.
noTriples.

eachRoundAtLeastOneTvMatch:- round(R), findall(match(S,T,R), tvMatch(S,T), Lits), atLeast(1,Lits), fail.
eachRoundAtLeastOneTvMatch.

notAtHome:- away(T,R), writeClause([-home(T,R)]), fail.
notAtHome.

noDoublesAtCertainRounds:- noDoubles(D), member(R,D), findall(double(T,R), team(T), Lits), exactly(0,Lits), fail.
noDoublesAtCertainRounds.
