...
writeClauses(MaxCost):-
    twoTeamsExactlyOneRound,
    eachTeamExactlyOneMatch,
    createHome,
    noMatchAtHome,
    createDouble,
    noDoublesOnCertainRounds,
    notTriples,
    eachRoundAtLeastTwoTvTeams,
    minDoubles(MaxCost),
    true,!.
...
twoTeamsExactlyOneRound:-
    difTeams(T1,T2), 
    findall(match(T1,T2,R), round(R), LitsHome),
    findall(match(T2,T1,R), round(R), LitsAway),
    append(LitsHome,LitsAway,Lits),
    exactly(1,Lits),
    fail.
twoTeamsExactlyOneRound.
...
eachTeamExactlyOneMatch:-
    team(T), round(R),
    findall(match(T,S,R), difTeams(S,T), LitsHome),
    findall(match(S,T,R), difTeams(S,T), LitsAway),
    append(LitsHome,LitsAway,Lits),
    exactly(1,Lits),
    fail.
eachTeamExactlyOneMatch.
...
createHome:- 
    team(T), round(R),
    findall(match(T,S,R), difTeams(T,S), Lits),
    expressOr(home(T,R),Lits),
    fail.
createHome.

noMatchAtHome:- away(T,R), writeClause([-home(T,R)]), fail.
noMatchAtHome.

createDouble:-
    team(T), round(R), Rant is R-1, round(Rant),
    writeClause([ -home(T,Rant), -home(T,R),  double(T,R) ]),
    writeClause([  home(T,Rant),  home(T,R),  double(T,R) ]),
    writeClause([ -home(T,Rant),  home(T,R), -double(T,R) ]),
    writeClause([  home(T,Rant), -home(T,R), -double(T,R) ]),
    fail.
...
noDoublesOnCertainRounds:- 
    team(T), noDoubles(ND), member(R,ND), 
    writeClause([ -double(T,R) ]),
    fail.
noDoublesOnCertainRounds.

notTriples:-
    team(T), round(R), Rant is R-1, round(Rant),
    writeClause([ -double(T,Rant), -double(T,R) ]),
    fail.
notTriples.
