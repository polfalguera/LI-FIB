...
writeClauses:- 
    teamExactlyKStudents,
    eachStudentsChosenFriends,
    differentExpertiseTopics,
    true.
writeClauses:- told, nl, write('writeClauses failed!'), nl,nl, halt.

teamExactlyKStudents:- numMembersTeam(K), findall(chosen(S), student(S), Lits), exactly(K, Lits), fail.
teamExactlyKStudents.

eachStudentsChosenFriends:- student(S1), student(S2), S1 \= S2, not(friends(S1,S2)), writeOneClause([-chosen(S1),-chosen(S2)]), fail.
eachStudentsChosenFriends.

differentExpertiseTopics:- student(S1), student(S2), S1 \= S2, expertise(S1,E), expertise(S2,E), writeOneClause([-chosen(S1),-chosen(S2)]), fail.
differentExpertiseTopics.
