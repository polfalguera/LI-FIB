...
writeClauses:-  
    exactlyOneTripPerBusAndDay,                % Cada dia un bus fa exactament un trajecte
    cityOfArrivalNextCityOfDeparture,          % En cas que no sigui el dia 1, un trajecte s'inicia des de la ciutat on va acabar l'anterior.
    eachTripAtMostOncePerDay,                  % Cada trajecte te lloc com a màxim un cop al dia
    eachTripAtLeastItsFreqPerWeek,             % Cada trajecte te lloc com a mínim N dies a la setmana
    eachBusAtMostMaxDistanceInConsecutiveDays, % Cada bus no pot recorrer mes d'una distancia maxima en dos dies consecutius
    true,!.
...
cityOfArrivalNextCityOfDeparture:- bus(B), day(D), D \= 1, LD is D - 1, trip(C1-C2), trip(C3-C4), C3 \= C2,
                                    writeOneClause([-bdcc(B,LD,C1,C2),-bdcc(B,D,C3,C4)]), fail.
cityOfArrivalNextCityOfDeparture.

eachTripAtMostOncePerDay:- trip(C1-C2), day(D), findall(bdcc(B,D,C1,C2), bus(B), Lits), atMost(1,Lits), fail.
eachTripAtMostOncePerDay.

eachTripAtLeastItsFreqPerWeek:- trip(C1-C2), frequency(C1-C2,F), findall(bdcc(B,D,C1,C2), (bus(B),day(D)), Lits), atLeast(F,Lits), fail.
eachTripAtLeastItsFreqPerWeek.

eachBusAtMostMaxDistanceInConsecutiveDays:- bus(B), consecutiveDays(D1,D2), tooLongDist(C1-C2-C3), writeOneClause([-bdcc(B,D1,C1,C2),-bdcc(B,D2,C2,C3)]), fail.
eachBusAtMostMaxDistanceInConsecutiveDays.
