...
%%% CHECKS FOR EACH SMALL SQUARE IT DOES NOT OVERLAP WITH ANY OF ALL THE OTHER SMALL SQUARES %%%
   %%%           It does not work, I'm still checking which could be the issue.          %%%
   %%% It gives stack limit exceeded as error, so I might not have to check all of them. %%%

% nonoverlapping(_,Sides,RowVars,ColVars):-
%     append(S1,[S|S2],Sides),
%     append(R1,[R|R2],RowVars),
%     append(C1,[C|C2],ColVars),
%     nth0(Si,Sides,S),
%     nth0(Ri,RowVars,R),
%     nth0(Ci,ColVars,C),
%     %write(Si), write(Ri), write(Ci), nl,
%     Si == Ri, Ri == Ci,
%     %write('Succesfully! Indexes are identical'), nl,
%     append(S1,S2,OtherSides),
%     append(R1,R2,OtherRowVars),
%     append(C1,C2,OtherColVars),
%     checkOverlapping(S,R,C,OtherSides,OtherRowVars,OtherColVars).

% checkOverlapping(_,_,_,[],[],[]).
% checkOverlapping(S,R,C,[S1|OS],[R1|OR],[C1|OC]):-
%     S #\= S1, R #\= R1, C #\= C1,
%     check(S,R,C,S1,R1,C1),
%     checkOverlapping(S,R,C,[S1|OS],[R1|OR],[C1|OC]).