...
satVariable( hasTower(V) ):- village(V). % means "village V has a tower"
...
writeClauses(MaxNumTowers):-
    eachVillageAtMostOneTower, % Restricció que cada village té com a máxim una torre
    hasTowerVillage,
    noTowerOutOfVillage,
    maxNumTowers(MaxNumTowers),
    eachSignificantVillageExactlyOneTower,
    everyVillageSupervised, 
    true,!.                    % this way you can comment out ANY previous line of writeClauses
writeClauses(_):- told, nl, write('writeClauses failed!'), nl,nl, halt.

eachVillageAtMostOneTower:- village(V), findall(towerPos(I,J), posVillage(V,I,J), Lits), atMost(1,Lits), fail.
eachVillageAtMostOneTower.

hasTowerVillage:- village(V), findall(towerPos(I,J), posVillage(V,I,J), Lits), expressOr(hasTower(V),Lits), fail.
hasTowerVillage.

noTowerOutOfVillage:- findall(towerPos(I,J), (position(I,J),not(posVillage(_V,I,J))), Lits), exactly(0,Lits), fail.
noTowerOutOfVillage.

maxNumTowers(MaxNumTowers):- findall(hasTower(V), village(V), Lits), atMost(MaxNumTowers,Lits).

eachSignificantVillageExactlyOneTower:- significantVillage(V), findall(towerPos(I,J), posVillage(V,I,J), Lits), exactly(1,Lits), fail.
eachSignificantVillageExactlyOneTower.

everyVillageSupervised:- village(V), findall(towerPos(I,J),posWatchesVillage(V,I,J), Lits), atLeast(1,Lits), fail.
everyVillageSupervised.
...
% Cost: Número de torres col·locades en la sol·lució M donada.
...
main:-
        ...
        shell('/mnt/c/Users/Msi/c/Users/Msi/Documents/ENGINYERIA_INFORMATICA/A3-Q2/LI-FIB/Lab/Practica_4/kissat-master/build/kissat -v infile.cnf > model', Result),  % if sat: Result=10; if unsat: Result=20.
        treatResult(Result,[]),!.
