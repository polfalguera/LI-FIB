...
onlyOneTaskAtATime:- gangster(G), task(T1), task(T2), T1 \= T2, hour(H), writeOneClause([-does(G,T1,H), -does(G,T2,H)]), fail.
onlyOneTaskAtATime.

noConsecutiveDifferentTasks:- gangster(G), task(T1), task(T2), T1 \= T2, hour(H1), hour(H2), H2 is H1 + 1, writeOneClause([-does(G,T1,H1),-does(G,T2,H2)]), fail.
noConsecutiveDifferentTasks.

gangsterNoAvailable:- gangster(G), task(T), blocked(G,H), writeOneClause([-does(G,T,H)]), fail.
gangsterNoAvailable.

atMostMaxConsecutiveHours(MaxConsecutiveHours):- gangster(G), task(T), hour(H1), hour(H2), H2 is H1 + MaxConsecutiveHours, findall(-does(G,T,H), between(H1,H2,H), Lits), writeOneClause(Lits), fail.
atMostMaxConsecutiveHours(_).

allTasksDone:- task(T), hour(H), needed(T,H,N), findall(does(G,T,H), available(G,H), Lits), exactly(N,Lits), fail.
allTasksDone.
...
costOfThisSolution(M,Cost):-
    between(0, 72, H), Cost is 72-H,
    gangster(G),
    costOfThisGangster(M,G,Cost), !.
